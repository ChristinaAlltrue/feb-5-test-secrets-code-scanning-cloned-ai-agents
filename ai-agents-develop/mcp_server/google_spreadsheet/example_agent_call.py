...
async def client():
    ...
    result = await agent.run(
        "I have a Spreasheet 'MCP test spreadsheets', read the first row, there should be animal categories, generate 3 breeds for each category and append under each category cell"
    )
    print(result.output)
