...
from app.core.agents.action_prototype.custom_questionnaire_assistant_v2.tool import (
    custom_questionnaire_assistant_v2,
)
...
@dataclass
class CustomQuestionnaireAssistantV2(BaseNode[State]):
    async def run(self, ctx: GraphRunContext[State, GraphDeps]) -> BaseNode:
        ...
        result_dict = None
        try:
            current_deps = CustomQuestionnaireAssistantV2NodeDeps.model_validate(
                ctx.deps.get_current_deps(ctx.state.output)
            )
            logfire.info(
                "Resolved input deps",
                deps=current_deps.model_dump(exclude={"google_token"}),
            )

            async with patched_action_deps(ctx, action_deps) as new_ctx:
                res = await custom_questionnaire_assistant_v2(
                    ctx=new_ctx,
                    google_token=current_deps.google_token,
                    sheet_name=current_deps.sheet_name,
                    context_document_name=current_deps.context_document_name,
                    goal=current_deps.goal,
                )
            logfire.info(f"Result: {res}")

            result = CustomQuestionnaireAssistantV2Output.model_validate(res)
            ...
            logfire.error(f"Error in Custom Questionnaire AssistantV2 Action: {e}")
            if result_dict is None:
                result_dict = {
                    "feedback": "Problem running the CustomQuestionnaireAssistantV2 Agent",
                    "reason": str(e),
                }
            result_with_feedback = await store_action_execution(result_dict, ctx)
            await action_deps.update_action_status(
