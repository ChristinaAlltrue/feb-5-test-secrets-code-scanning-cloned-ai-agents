...
from app.core.agents.action_prototype.sample_generic_action.tool import sample_increment
...
@dataclass
class SampleGenericAction(BaseNode[State]):
    async def run(self, ctx: GraphRunContext[State, GraphDeps]) -> BaseNode:
        ...
        try:
            # 3. Get the current deps and validate it
            current_deps = SampleGenericActionDeps.model_validate(
                ctx.deps.get_current_deps(ctx.state.output)
            )
            logfire.info(f"Input: {current_deps}")

            interpreted_result = interpret_single_function_parameters(
                sample_increment,
                current_deps.goal,
                previous_results=str(ctx.state.output),
            )
            logfire.info(f"Interpreted result: {interpreted_result}")

            # Extract the parameters from the interpreted result
            if (
                interpreted_result.get("interpreted_parameters")
                and len(interpreted_result["interpreted_parameters"]) > 0
            ):
                interpreted_parameters = interpreted_result["interpreted_parameters"][
                    0
                ]["parameters"]
                logfire.info(f"Extracted parameters: {interpreted_parameters}")
            else:
                raise ValueError("No parameters could be interpreted from the goal")

            # 4. Run the logic
            async with patched_action_deps(ctx, action_deps) as new_ctx:
                params = {"ctx": new_ctx, **interpreted_parameters}

                res = await sample_increment(**params)

            # 5. Store the output
            result = SampleGenericActionOutput(output=res)
            ...
            logfire.error(f"Error in SampleGenericAction action: {e}")
            # 8. Handle errors and update the action status to failed
            await action_deps.update_action_status(
