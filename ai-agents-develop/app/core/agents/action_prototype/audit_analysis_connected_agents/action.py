...
from app.core.agents.action_prototype.audit_analysis_connected_agents.tool import (
    run_connected_agent,
)
...
@dataclass
class AuditAnalysisConnectedAgents(BaseNode[State]):
    async def run(self, ctx: GraphRunContext[State, GraphDeps]) -> BaseNode:
        logfire.info("Running AuditAnalysisConnectedAgents action")
        ...
        try:
            ...
            async with patched_action_deps(ctx, action_deps) as new_ctx:
                result = await run_connected_agent(
                    ...
                    user_list_instructions=current_deps.user_list_instructions,
                )
            ...
            ctx.state.data = {
                "analysis_agent_generated_file": result.analysis_agent_generated_file
            }
