...
def get_sample_two_action_pause():
    """Factory function that generates fresh UUIDs each time it's called."""
    return {
        "sample two action pause": PostControlExecutionRequest(
            ...
            **SKIP COMPLIANCE CHECK** Execute two supervisor agent actions in sequence.
            ...
            entity_exec_args=[
                {
                    "credentials": {},
                    "task_description": "Check if there are any emails related with Task 001",
                    "additional_description": f"if there is an email said this task has to pause, then Pause. Google token used for email tool is {GOOGLE_CREDENTIALS}",
